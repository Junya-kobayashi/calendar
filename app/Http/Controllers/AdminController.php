...
class AdminController extends Controllers
{
  ...
  public function doCreateCalendar(Request $request, Calendar $calendar)
  {
    ...
    return redirect('/calendar/create')
      -> with('message', ['type' => 'success', 'text' => 'Calendar was created']);
  }
  ...
  public function doCreateEvent(Event $env, Request $request)
  {
    $this -> validate($request, [
      'title' => 'required',
    ...
    $env -> event_id = $created_event->id;
    $env -> datetime_start = $start_datetime->toDateTimeString();
    $env -> datetime_end = $end_datetime->toDateTimeString();
    $env -> save();

    return redirect('/event/create')
      -> with('message', ['type'=>'success', 'text'=>'Event was Created']);
  }
  ...
  public function doSyncCalendar(Request $request)
  {
    ...
    $g_calendar = $g_cal -> calndars -> get($g_calendar_id);
    ...
    $googlecalendar_events = $g_cal->evnets->listEvents($g_calendar_id, $params);

    while(true) {
      foreach($googlecalendar_events->getItems() as $g_event) {
        $g_event_id = $g_event->id;
        $g_event_title = $g_event->getSummury();
    ...
    return redirect('/calendar/sync')
      -> with('message',['type' => 'success', 'text' => 'Calendar was synced']);
  }
